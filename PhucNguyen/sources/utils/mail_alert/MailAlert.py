...
class MailAlert():
    ...
    def send_alert(self):
        ...
        msg["Subject"] = Header(
            u'[AII20] [FaceCheckIn] FCI CHECK IN MAIL', 'utf-8')
        ...
        imagePaths = list(paths.list_images(self.alert_path))
        msg_html = u'<h1>FCI CHECK IN</h1>'
        with open(f"{self.alert_path}/info.json", 'r') as fp:
            data = json.load(fp)
        if data['name'] == 'Unknown':
            message = "UNKNOWN ALERT!!!"
        else: 
            message = f"{data['name']} check in at {data['time']}, {data['place']}"
        for (i, img_path) in enumerate(imagePaths):
            image = dict(title=f"image {i}",
                         path=img_path, cid=str(uuid.uuid4()))
            msg_html += '<h3>' + message + '</h3><div dir="ltr">''<img src="cid:{cid}" alt="{alt}"><br></div>'.format(
                alt=cgi.html.escape(image['title'], quote=True), **image)

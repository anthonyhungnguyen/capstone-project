...
def draw_frame(frame, frame_count, star_time, name_list, line_track, trackers):
    """
    Draw frame
    Args:
        TBU
    ...
    for tracker in trackers:
        text = "Track ID: " + str(int(tracker[-1]))
        frame = cv2.putText(frame, text, (int(tracker[0]), int(tracker[1])), cv2.FONT_HERSHEY_COMPLEX,1,(0,255,255),1)
    name_pos = 50
    for key in name_list:
        name = name_list[key]['name']
        if name != UNKNOWN:
            if name != "Unknown ALERT":
                # frame = cv2.putText(frame, "Welcome {} to HaNoi Room !".format(
                frame = cv2.putText(frame, "Welcome {} to Bach Khoa !".format(
...
class Camera(BaseCamera):
    ...
    def frames():
        ...
        SKIP_FRAME_DETECTING = 6
        SKIP_FRAME_RECOGNIZING = 24
        SKIP_FRAME_TRACKING = 3
        FACE_PADDING = 15
        ...
        line_track = {}
        frame_number = 0
        jsonfile = open('./meta/tracking_meta.json', 'w')
        if DISPLAY:
        ...
        while True:
            ...
            for d in dets:
                ...
                face = np.array([[left, top, right, bottom]])
                if flag == False:
                    trackerfaces = face
                    flag = True
                else:
            ...
            if flag == True:
                trackers = mAILIBS.TRACKERS(trackerfaces)
                frame_number += 1
            ...
            # print("%%%%%%TRACKER trackerfaces: ",trackerfaces)
            ...
            #     mCONFIG.LOGGER.info("Send mail alert...")

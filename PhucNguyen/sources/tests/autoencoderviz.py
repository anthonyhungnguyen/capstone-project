...
def encoders_post_proccessing(model, unknown, X_test, person=None):
    ...
    dataviz_y = []
    plt.figure()
    ailibs_autoencoder = os.path.join(PYTHON_PATH, "ailibs_data", "testing", "autoencoder")
    # get max value of person score
    for face in X_test:
        pred = model.predict(face)
        res = np.sqrt(metrics.mean_squared_error(pred, face))
        person_scorelist.append(res)
    dataviz_x = person_scorelist
    dataviz_y = [1]*len(person_scorelist)
    ...
    dataviz_x += unknown_scorelist
    dataviz_y = dataviz_y + [0]*len(unknown_scorelist)
    ...
    plt.scatter(dataviz_x, dataviz_y)

    # compute parameters alpha, beta for auto encoder threshold
    B = 9.12/(unknown_score-person_score)
    A = -4.6 - B*person_score
    N = 100
    X = np.linspace(min(person_scorelist), max(unknown_scorelist), N, True)
    y = sigmoid_function(A, B, X)
    plt.plot(X,y)
    plt.title(person)
    plt.xlabel("Mean Square Error")
    plt.ylabel("Score")
    plt.savefig(f"{ailibs_autoencoder}/{person}.jpg")
    weight = {}
    weight["A"] = A
    weight["B"] = B

    plt.figure()
    plt.hist(person_scorelist, bins='auto')  
    plt.hist(unknown_scorelist, bins='auto')  
    plt.title(f"Histogram for {person} vs Unknown")
    plt.savefig(f"{ailibs_autoencoder}/{person}_hist.jpg")
